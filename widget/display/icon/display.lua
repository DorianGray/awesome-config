...
local function draw_display(surface)
  ...
  cr:rectangle(surface.width * 0.7, 0, surface.width * 0.4, surface.height * 0.4)
  cr:fill()
end

local function draw_arrow(surface)
  local cr = lgi.cairo.Context(surface)
  cr:set_fill_rule(lgi.cairo.FillRule.EVEN_ODD)
  cr:set_source(color(beautiful.fg_normal))
  local centerx, centery = surface.width / 2, surface.height / 2
  cr:set_line_width(math.min(surface.height, surface.width) * 0.05)
  cr:move_to(centerx * 1.2, centery)
  cr:line_to(surface.width * 0.95, surface.height * 0.15)
  cr:line_to(surface.width * 0.6, surface.height * 0.15)
  cr:move_to(surface.width * 0.95, surface.height * 0.15)
  cr:line_to(surface.width * 0.95, surface.height * 0.5)
  cr:stroke()
end
