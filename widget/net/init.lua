...
local function get_net_status()
  ...
  local function ping()
    local output = process.run('ping -c 1 -W 1 8.8.8.8 | grep \'100% packet loss\'').stdout()
...
local function get_local_interfaces()
  local output = process.run('ip link').stdout()
...
local function get_wifi_link(iface)
  ...
  local wpa_output = process.run('wpa_cli status').stdout()
  local info = {}
...
function net:get_area_wifi(iface)
  local scan_output = process.run('wpa_cli abort_scan && wpa_cli scan').stdout()
  ...
  if not scan_timer or not scan_timer.started then
    scan_timer = gears.timer({
      ...
      autostart = false,
      callback  = function()
        local output = process.run('wpa_cli scan_results').stdout()
...
function net:toggle_wifi(iface)
  local output = process.run('sudo rfkill -r -n').stdout()
  local rfkill = {}
  for line in output:gmatch("[^\r\n]+") do
    local interface = {}
  ...
  for _, interface in pairs(rfkill) do
    ...
    if wtype == 'wlan' then
      ...
      process.run('sudo rfkill '..action..' '..wtype).stdout()
      self:menu()
    end
  end
end
