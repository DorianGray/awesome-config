...
local function run_once(cmd, match)
  ...
  awful.util.spawn_with_shell('pgrep -u $USER -x ' .. match .. ' > /dev/null || (' .. cmd .. ')')
end
...
local modkey     = 'Mod4'
local altkey     = 'Mod1'
local terminal   = 'gnome-terminal --disable-factory --name=\'normal\' ' or 'urxvtc'
...
local volumeupdatetimer = timer({ timeout = 0.5 }) -- set update interval in s
...
for s = 1, screen.count() do
  ...
  right_layout_add(memicon, memwidget, cpuicon, cpuwidget)
...
awful.key({ altkey }, 'p', function() awful.util.spawn_with_shell('gnome-screenshot -c') end),
